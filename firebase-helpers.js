...
export async function getResources(pathKey) {
  const q = query(collection(db, "resources"), where("pathKey", "==", pathKey));
...
export async function getFolders(pathKey) {
  const q = query(collection(db, "folders"), where("pathKey", "==", pathKey));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function getComments(resourceId) {
  const q = query(collection(db, "comments"), where("resourceId", "==", resourceId));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
